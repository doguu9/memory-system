...
#only add imgs that are returned by nearpy
        ...
        new = list(val_dataset._class_labels)
        pred_arr = []
        dimension = 9984
        ...
        for img in new:
        #nea rpy stuff
            folder1 = os.path.join(os.path.join(config['dataset_path'], 'originals2'), img)
            im = os.path.join(folder1, 'view_000000.npz')
        ...
        for item in pred_arr:
            ...
            f = open("ground.txt", "a")
            if item[0] == item[1]:
                f.write('1 {} {} '.format(item[0], item[1]))
            else:
                f.write('0 {} {} '.format(item[0], item[1]))
            f.close()

#run prediction generator as normal
